...
func (widget *rssWidget) fetchItemsFromFeedTask(request rssFeedRequest) ([]rssFeedItem, error) {
	...
	for i := range feed.Items {
		...
		if item.GUID != "" {
    		rssItem.Link = rssItem.Link + "#" + url.QueryEscape(item.GUID)
		} else if rssItem.Title != "" {
    		rssItem.Link = rssItem.Link + "#" + url.QueryEscape(
        		fmt.Sprintf("%s-%s", rssItem.Title, rssItem.PublishedAt.Format(time.RFC3339)),
    		)
		}
